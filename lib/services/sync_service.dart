...
class SyncService {
  ...
  Future<void> synchronizeFlights() async {
    ...
    try {
      ...
      for (final remoteFlight in remoteFlights) {
        final localFlight = localFlightsMap[remoteFlight.id]; // Find local by remoteID
        ...
        if (localFlight == null) {
          ...
          logDebug(
              'Comparing flight #${remoteFlight.id}: Remote editedAt: $remoteEditedAt, Local editedAt: $localEditedAt');
          ...
          if (remoteEditedAt > localEditedAt) {
            ...
            final companion =
                _apiFlightToDrift(remoteFlight).copyWith(id: Value(localFlight.id));
      ...
      for (final localFlight in localFlights) {
        final remoteFlight = remoteFlightsMap[localFlight.remoteID?.toString()]; // Find remote by local's remoteID

        final localEditedAt = localFlight.editedAt?.millisecondsSinceEpoch ?? 0;
        final localSyncedAt = localFlight.syncedAt?.millisecondsSinceEpoch ?? 0;

        logDebug(
            'Processing local flight #${localFlight.id}: remoteID: ${localFlight.remoteID}, localEditedAt: $localEditedAt, localSyncedAt: $localSyncedAt');

        if (localFlight.remoteID == null) {
          // Case C: New local flight (no remoteID). Create on server.
          logDebug('New local flight #${localFlight.id} found. Uploading to server.');
          try {
            final apiData = await _driftFlightToApi(localFlight);
            logDebug('API data for new flight #${localFlight.id}: $apiData');
            final newRemoteFlight =
                await _apiService.createFlight(apiData);
            
            logDebug(
                'Server response for new flight #${localFlight.id}: id: ${newRemoteFlight.id}, editedAt: ${newRemoteFlight.editedAt}');

            // Update local record with remoteID and set sync times
            final companion = localFlight.toCompanion(true).copyWith(
                  remoteID: Value(int.parse(newRemoteFlight.id)),
                  syncedAt: Value(newRemoteFlight.editedAt),
                  editedAt: Value(newRemoteFlight.editedAt), // Ensure editedAt is also updated to match remote after creation
                );
            await _driftService.saveFlight(companion);
            logDebug('Local flight #${localFlight.id} updated with remote ID and sync times.');
          } catch (e) {
            logDebug('Failed to create flight #${localFlight.id} on server: $e');
          ...
          logDebug('Local flight #${localFlight.id} has remoteID but no remote counterpart. Attempting to re-create on server.');
          try {
               final newRemoteFlight = await _apiService.createFlight(await _driftFlightToApi(localFlight));
               logDebug(
               ...
               await _driftService.saveFlight(localFlight.toCompanion(true).copyWith(
                 remoteID: Value(int.parse(newRemoteFlight.id)),
                 syncedAt: Value(newRemoteFlight.editedAt),
                 editedAt: Value(newRemoteFlight.editedAt),
               ));
               logDebug('Local flight #${localFlight.id} re-created and updated with remote ID and sync times.');
          } catch (e) {
            logDebug('Failed to re-create flight #${localFlight.id} on server: $e');
          }
        }
        else {
          // Case E: Local flight exists and has a remote counterpart. Compare timestamps for updates.
          final remoteEditedAt = remoteFlight.editedAt?.millisecondsSinceEpoch ?? 0;

          logDebug(
              'Comparing local flight #${localFlight.id} for upload: localEditedAt: $localEditedAt, localSyncedAt: $localSyncedAt, remoteEditedAt: $remoteEditedAt');

          if (localEditedAt > localSyncedAt && localEditedAt >= remoteEditedAt) {
            // Local is newer than its last synced state AND newer or equal to remote. Upload.
            logDebug('Local flight #${localFlight.id} has been updated locally. Uploading changes.');
            try {
              final apiData = await _driftFlightToApi(localFlight);
              logDebug('API data for updating flight #${localFlight.id}: $apiData');
              await _apiService.updateFlight(localFlight.remoteID.toString(), apiData);
              // After successful upload, update local syncedAt to match localEditedAt
              await _driftService.saveFlight(localFlight.toCompanion(true).copyWith(
                syncedAt: Value(localFlight.editedAt),
              ));
              logDebug('Local flight #${localFlight.id} syncedAt updated after successful upload.');
            } catch (e) {
              logDebug('Failed to update flight #${localFlight.id} on server: $e');
              // Optionally, mark local flight as failed to sync or retry later
            }
          }
        }
  ...
  T _getEnumValue<T extends Enum>(List<T> values, String? name, T defaultValue) {
  ...
  FlightsCompanion _apiFlightToDrift(api_flight.Flight flight) {
    return FlightsCompanion(
      ...
      flightDuration: Value(flight.arrivalTime.difference(flight.departureTime)),
      distance: Value(flight.distance.toInt()),
      routePath: Value(flight.routePath
          .map((e) => db.RoutePoint(latitude: e.latitude, longitude: e.longitude))
          .toList()),
      directRoutePath: Value(flight.directRoutePath
          .map((e) => db.RoutePoint(latitude: e.latitude, longitude: e.longitude))
          .toList()),
      ...
      seatType: Value(_getEnumValue(db.SeatType.values, flight.seatType, db.SeatType.none)),
      flightClass: Value(_getEnumValue(db.FlightClass.values, flight.flightClass, db.FlightClass.none)),
      flightReason: Value(_getEnumValue(db.FlightReason.values, flight.flightReason, db.FlightReason.none)),
      editedAt: Value(flight.editedAt),
      syncedAt: Value(flight.editedAt), // When pulling from remote, it's synced by definition
    );
  }
  ...
  Future<Map<String, dynamic>> _driftFlightToApi(Flight flight) async {
    return {
      ...
      'flight_date': flight.flightDate.toString(), // RESPECT THIS
      'flight_duration': flight.flightDuration.toString(),
      'distance': flight.distance,
      'route_path': jsonEncode(flight.routePath.map((e) => e.toJson()).toList()),
      'flight_number': flight.flightNumber,
      'airplane_type': flight.airplaneType,
      'airplane_registration': flight.registration,
      'seat': flight.seat,
      'seat_type': flight.seatType == db.SeatType.none ? '' : flight.seatType?.name,
      'flight_class': flight.flightClass == db.FlightClass.none ? '' : flight.flightClass?.name,
      'flight_reason': flight.flightReason == db.FlightReason.none ? '' : flight.flightReason?.name,
    };
  }
}
